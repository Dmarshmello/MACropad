...
// Button layout based on your numbering scheme
...
// Define multimedia key codes for volume control
...
void loop() {
    ...
    for (int col = 1; col < numCols; col++) { // Start from col=1 because col=0 is the encoder
      ...
      if (digitalRead(colPins[col]) == LOW) { // Button pressed
        if (keyState[row][col] == false && (currentTime - lastDebounceTime[row][col] > debounceTime)) {
          ...
          Keyboard.print(keys[row][col]); // Send the corresponding number via keyboard
  ...
  if (newPosition != oldPosition) {
    if (newPosition > oldPosition) {
      Keyboard.press(VOLUME_UP); // Increase volume
      delay(100);
      Keyboard.release(VOLUME_UP);
    } else {
      Keyboard.press(VOLUME_DOWN); // Decrease volume
      delay(100);
